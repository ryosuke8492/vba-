...
' ---------------------------------------------------------------
' 区切り文字の一覧（優先順位順）
'   新しい区切り文字に対応したくなったら、ここに追記するだけでよい。
'   例：全角カンマにも対応したい → Array("～", "~", "-", "、") のように追加
' ---------------------------------------------------------------
Private Function GetSeparators() As Variant
    GetSeparators = Array("～", "~", "-")
End Function

' ---------------------------------------------------------------
' input の中から、GetSeparators() の優先順位で最初に見つかった
' 区切り文字を探す。見つからなければ空文字 "" を返す。
' ---------------------------------------------------------------
Private Function FindSeparator(ByVal cleaned As String) As String
    Dim seps As Variant  ' 区切り文字候補一覧
    Dim i    As Long     ' 候補ループカウンタ

    seps = GetSeparators()

    For i = LBound(seps) To UBound(seps)
        If InStr(cleaned, seps(i)) > 0 Then
            FindSeparator = seps(i)
            Exit Function
        End If
    Next i

    FindSeparator = ""  ' 見つからなかった
End Function

Public Function ExtractEndDate(ByVal input As String) As Date
    ' ─── 変数宣言 ───────────────────────────────────────
    Dim cleaned   As String  ' トリム済み入力文字列
    Dim separator As String  ' 検出したセパレータ（GetSeparators() 参照）
    ...
    ' ── セパレータ検出（GetSeparators() の優先順位で検索）──
    separator = FindSeparator(cleaned)
...
' ---------------------------------------------------------------
' ---------------------------------------------------------------
' 対応する日付表記パターン一覧（正規表現の断片）
'   新しい表記に対応したくなったら、ここに1つ追記するだけでよい。
'   例：「7月22日」のような表記にも対応したい
'       → Array("\d{1,2}/\d{1,2}", "\d{1,2}", "\d{1,2}月\d{1,2}日") のように追加
' ---------------------------------------------------------------
Private Function GetDatePatterns() As Variant
    GetDatePatterns = Array( _
        "\d{1,2}/\d{1,2}", _  ' 例："7/22"（月/日）
        "\d{1,2}" _           ' 例："23"（日のみ。月省略時の終了日用）
    )
End Function

' ---------------------------------------------------------------
' GetDatePatterns() の各パターンを「|（または）」でつなげて
' 1つの正規表現文字列に組み立てる
' ---------------------------------------------------------------
Private Function BuildDatePattern() As String
    BuildDatePattern = Join(GetDatePatterns(), "|")
End Function
...
Public Function ExtractEndDateEx(ByVal input As String) As Date
    ...
    ' パターンは GetDatePatterns() で一覧管理し、BuildDatePattern() で
    ' "|"（または）でつなげた1本の正規表現にして使う
    re.Pattern = BuildDatePattern()
...
'
' 期限ルール一覧（DocRules で一括管理。ここを直せば列や日数を変更可能）
'   書類A：+1 営業日 / 書類B：+2 営業日 / 書類C：+3 営業日 / 書類D：+5 営業日
'   書類E：+7 日     / 書類F：+14 日     / 書類G：+30 日    / 書類H：+60 日
'==============================================================
...
' 判定方法の種類
'   ByDeadline  : 期限日以内ならOK（CheckItem 相当）
'   ByTolerance : 基準日 ±ToleranceDays 以内ならOK（CheckItemTolerance 相当）
...
' 書類1件分の判定ルール
Private Type DocRule
    SrcCol        As Long         ' 完了日が入っている列
    DstCol        As Long         ' 判定結果を書く列
    Kind          As DeadlineKind ' 期限の数え方（営業日 / カレンダー日）
    N             As Long         ' 作業日から何日後が基準日か
    Mode          As CheckMode    ' 判定方法（期限判定 / 許容日数判定）
    ToleranceDays As Long         ' Mode=ByTolerance のときだけ使う許容日数
End Type

' ---------------------------------------------------------------
' 判定ルールを配列で返す
'   列・日数・判定方法・許容日数を変えたい／書類を増減したい場合は
'   ここだけ直せばよい（メンテナンスはこの1関数に集約）
' ---------------------------------------------------------------
Private Function GetDocRules() As DocRule()
    Dim rules(1 To 8) As DocRule

    '            SrcCol,DstCol, Kind,     N,   Mode,        ToleranceDays
    rules(1) = MakeRule(2, 10, Business, 1)                 ' 書類A：+1営業日以内
    rules(2) = MakeRule(3, 11, Business, 2)                 ' 書類B：+2営業日以内
    rules(3) = MakeRule(4, 12, Business, 3)                 ' 書類C：+3営業日以内
    rules(4) = MakeRule(5, 13, Business, 5)                 ' 書類D：+5営業日以内
    rules(5) = MakeRule(6, 14, Calendar, 7, ByTolerance, 2)  ' 書類E：基準日 ±2日 許容
    rules(6) = MakeRule(7, 15, Calendar, 14)                ' 書類F：+14日以内
    rules(7) = MakeRule(8, 16, Calendar, 30)                ' 書類G：+30日以内
    rules(8) = MakeRule(9, 17, Calendar, 60)                ' 書類H：+60日以内

    GetDocRules = rules
End Function

Private Function MakeRule(srcCol As Long, dstCol As Long, _
                           kind As DeadlineKind, n As Long, _
                           Optional mode As CheckMode = ByDeadline, _
                           Optional toleranceDays As Long = 0) As DocRule
    Dim r As DocRule
    r.SrcCol = srcCol
    r.DstCol = dstCol
    r.Kind = kind
    r.N = n
    r.Mode = mode
    r.ToleranceDays = toleranceDays
    MakeRule = r
End Function

' ---------------------------------------------------------------
' ルールと作業日・祝日リストから基準日（期限日）を計算
' ---------------------------------------------------------------
Private Function CalcDeadline(rule As DocRule, workDate As Date, _
                               holidays As Variant) As Date
    If rule.Kind = Business Then
        CalcDeadline = AddBusinessDays(workDate, rule.N, holidays)
    Else
        CalcDeadline = AddCalendarDays(workDate, rule.N)
    End If
End Function

' ---------------------------------------------------------------
' ルールの Mode に応じて CheckItem / CheckItemTolerance を呼び分ける
' ---------------------------------------------------------------
Private Sub EvaluateRule(srcCell As Range, dstCell As Range, _
                          rule As DocRule, targetDate As Date)
    If rule.Mode = ByTolerance Then
        Call CheckItemTolerance(srcCell, dstCell, targetDate, rule.ToleranceDays)
    Else
        Call CheckItem(srcCell, dstCell, targetDate)
    End If
End Sub
...
Public Sub Main()
    ...
    Dim r        As Long       ' ルールループカウンタ
    Dim workDate As Date       ' 作業日（A列）
    Dim holidays As Variant    ' 祝日リスト
    Dim rules()  As DocRule    ' 書類ごとの判定ルール一覧
    ' ────────────────────────────────────────────────────
    ...
    For i = 2 To lastRow  ' 2行目からデータ
        ...
        ' 書類ルールを1件ずつループで判定
        ' （8書類分の個別Callも、判定方法ごとの分岐も不要に）
        For r = LBound(rules) To UBound(rules)
            Call EvaluateRule(ws.Cells(i, rules(r).SrcCol), _
                               ws.Cells(i, rules(r).DstCol), _
                               rules(r), _
                               CalcDeadline(rules(r), workDate, holidays))
        Next r
